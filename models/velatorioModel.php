...
class velatorioModel extends Model
{
	...
	public function getVelatorioPlan($plan){
		...
		$vel = $this->_db->prepare("SELECT v.id, v.tramites, v.asesor, v.sala, v.capilla, v.cirios, v.portacirios, v.cruces, v.florescanastos, v.florescubreurnas, v.condolencias, v.parroco, v.coro, v.avisosprensa, v.tarjetas, v.cafeteria, v.plan_id, v.componente_id, p.nombre as plan, img.nombre as imagen FROM velatorios as v INNER JOIN planes as p ON v.plan_id = p.id INNER JOIN imagenes as img ON img.plan_id = v.plan_id and img.componente_id = v.componente_id WHERE v.plan_id = ?");
