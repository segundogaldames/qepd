...
class imagenesController extends Controller
{
	...
	public function addComponentePlan($componente = null, $plan = null){
		$this->verificarSession();

		if (!$this->filtrarInt($componente)) {
			$this->redireccionar('planes');
		}
		...
		if (!$this->_componente->getComponenteId($this->filtrarInt($componente))) {
			$this->redireccionar('planes');
		}
		...
		if ($this->getInt('enviar') == 1) {
			...
			if (!$this->getSql('titulo')) {
				$this->_view->assign('_error', 'Debe ingresar el título de la imagen');
				$this->_view->renderizar('addComponentePlan');
				exit;
			}
